...
def upload_csv():
    try:
        ...
        # Check if any of the specified columns is missing in the DataFrame
        required_columns = ['ID', 'Pickup Date', 'Origin City', 'Origin State', 'Delivery City', 'Delivery State', 'Origin KMA', 'Delivery KMA', 'Miles']
        missing_columns = [col for col in required_columns if col not in df.columns]

        if missing_columns:
            return f"The following column(s) are missing: {', '.join(missing_columns)}", 400
       ...
       # if 'Carrier Pay' not in df.columns:
        ...
        df['LANE'] = df['Origin City'] + [', '] * df.shape[0] + df['Origin State'] + ['-'] * df.shape[0] + df[
            'Delivery City'] + [', '] * df.shape[0] + df['Delivery State']
